...
Download the latest release from [Releases](https://github.com/Evilander/evero-releases/releases):
...
# Run the build pipeline (copies from mac source + applies all 13 patches)
...
EveRo patches the original roro Electron app's minified bundles using a 13-step build pipeline. Each step is a Node.js script that applies targeted string replacements.
...
Runs 100 automated checks across main process, renderer, preload, files, config, and color palette.
...
4. Run `node scripts/verify-all.js` after changes (must be 100/100)
